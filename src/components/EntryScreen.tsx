      ...
      <div className="relative z-10 animate-float">
        <div className="relative w-[400px] md:w-[500px] mx-4 aspect-[4/5]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-16 py-24 md:px-24 md:py-32">
            ...
            {/* تم تصغير مساحة الأزرار هنا لتأخذ 75% فقط من العرض */}
            <div className="space-y-3 w-[85%] md:w-[75%]">
              <button
                onClick={() => { playSound('click'); setShowRegister(true); }}
                className="w-full py-2 px-3 rounded-xl font-bold text-sm md:text-base bg-primary hover:bg-primary/80 text-primary-foreground transition-all duration-200 hover:scale-105 shadow-lg shadow-primary/30"
              >
                🚀 تسجيل كلاعب
              </button>
              <button
                onClick={() => { playSound('click'); setShowAdmin(true); }}
                className="w-full py-2 px-3 rounded-xl font-bold text-sm md:text-base bg-destructive hover:bg-destructive/80 text-destructive-foreground transition-all duration-200 hover:scale-105 shadow-lg shadow-destructive/30"
              >
                🔐 دخول كأدمن
              </button>
            </div>
